...
export const runAssistant = async ({
  ...
  //Runのステータスの確認
  export const chaeckRunStateAndGetMessage = async({
    threadId,
    runId,
  }:{
    threadId:string
    ...
    try{
      const {status} = await openAiClient.beta.threads.runs.retrieve(threadId,runId);

      if(
        status ==='failed' ||
        status ==='requires_action' || // ディベートの返答を返すので
        status ==='cancelling' ||
        status ==='cancelled' ||
        status ==='incomplete' ||
        status ==='expired'
      ...
      if(messageContent.type!=='text') throw new Error(`MessageContent's type is ${messageContent.type}`);
      ...
      return {
        result:false,
        message:`Failed to checkRunStateAndGetMessage.${err.message}`,
        data:null
      }
